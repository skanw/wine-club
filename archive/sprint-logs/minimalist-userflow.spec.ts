...
test.describe('Minimalist Wine Club - Complete User Flow', () => {
  ...
  test('Navigation and routing', async ({ page }) => {
    ...
    for (const link of navLinks) {
      const navLink = page.locator(`text=${link.text}`);
  ...
  test('Error handling and edge cases', async ({ page }) => {
    ...
    try {
      await page.goto('http://localhost:3000/minimalist', { timeout: 5000 });
    } catch (error) {
      // Expected to fail due to aborted requests
