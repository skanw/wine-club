...
function getS3KeyForCave(fileName: string, caveId: string) {
  const ext = path.extname(fileName).slice(1);
  return `caves/${caveId}/logos/${randomUUID()}.${ext}`;
}
...
export const getUploadFileSignedURLFromS3ForCave = async ({ fileName, fileType, caveId }: S3UploadCave) => {
  const key = getS3KeyForCave(fileName, caveId);
