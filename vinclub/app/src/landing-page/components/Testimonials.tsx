...
export default function Testimonials({ testimonials }: { testimonials: Testimonial[] }) {
  ...
  return (
    <div className='mx-auto mt-32 max-w-7xl sm:mt-56 sm:px-6 lg:px-8'>
      <SectionTitle title='Ce que disent nos utilisateurs' />
              ...
              <CardFooter className='pt-0 flex flex-col'>
                <div className='flex items-center gap-x-3 w-full'>
                  <div className='min-w-0 flex-1'>
                    <CardTitle className='text-sm font-semibold transition-colors duration-200 truncate'>
                      {testimonial.name}
                    </CardTitle>
      ...
      {shouldShowExpand && (
        <div className='flex justify-center mt-8 md:hidden'>
          ...
          >
            {isExpanded ? 'Voir moins' : `Voir ${testimonials.length - mobileItemsToShow} de plus`}
          </button>
        </div>
      )}
    </div>
