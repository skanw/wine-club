...
import { WineBoxStatus } from '@prisma/client';
...
export const stripeWebhook: PaymentsWebhook = async (request, response, context) => {
  try {
    ...
    // Handle events - check if they're VinClub subscriptions first, otherwise handle as SaaS app subscriptions
    switch (eventName) {
      case 'checkout.session.completed':
        await handleCheckoutSessionCompleted(data, prismaUserDelegate);
        break;
      case 'invoice.paid':
        // Try VinClub subscription first, fallback to SaaS subscription
        const vinClubHandled = await handleVinClubInvoicePaid(data, context);
        if (!vinClubHandled) {
      ...
      case 'customer.subscription.updated':
        // Try VinClub subscription first, fallback to SaaS subscription
        const vinClubUpdated = await handleVinClubSubscriptionUpdated(data, context);
        if (!vinClubUpdated) {
      ...
      case 'customer.subscription.deleted':
        // Try VinClub subscription first, fallback to SaaS subscription
        const vinClubDeleted = await handleVinClubSubscriptionDeleted(data, context);
        if (!vinClubDeleted) {
      ...
      default:
        ...
        // handling above and that are necessary for the functioning of your app.
...
/**
 * Handle VinClub subscription invoice.paid event
 * Auto-creates WineBox records when payment succeeds
 * @returns true if handled as VinClub subscription, false otherwise
 */
async function handleVinClubInvoicePaid(
  invoice: InvoicePaidData,
  context: any
): Promise<boolean> {
  // Check if this is a VinClub subscription (has subscription metadata)
  ...
  // Find the VinClub subscription by Stripe subscription ID
  ...
  if (!subscription) {
    // Not a VinClub subscription, skip
    return false;
  }

  // Calculate billing cycle from invoice period (YYYY-MM format)
  const periodStart = new Date(invoice.period_start * 1000);
  const billingCycle = `${periodStart.getFullYear()}-${String(periodStart.getMonth() + 1).padStart(2, '0')}`;

  // Check if WineBox already exists for this billing cycle
  const existingWineBox = await context.entities.WineBox.findFirst({
    where: {
  ...
  if (existingWineBox) {
    // WineBox already exists, skip
    return true;
  }

  // Create WineBox record
  await context.entities.WineBox.create({
    data: {
      subscriptionId: subscription.id,
      billingCycle,
      status: WineBoxStatus.pending,
    },
  });

  console.log(`Created WineBox for subscription ${subscription.id}, billing cycle ${billingCycle}`);
  return true;
}

/**
 * Handle VinClub subscription.updated event
 * Updates subscription status based on Stripe subscription status
 * @returns true if handled as VinClub subscription, false otherwise
 */
async function handleVinClubSubscriptionUpdated(
  subscription: SubscriptionUpdatedData,
  context: any
): Promise<boolean> {
  const stripeSubscriptionId = subscription.id;
  
  const vinClubSubscription = await context.entities.Subscription.findFirst({
    where: {
      stripeSubscriptionId,
    },
  });

  if (!vinClubSubscription) {
    // Not a VinClub subscription, skip
    return false;
  }

  // Map Stripe subscription status to VinClub subscription status
  ...
  if (status) {
    await context.entities.Subscription.update({
      where: { id: vinClubSubscription.id },
      data: {
        status: status as any,
        updatedAt: new Date(),
        ...(status === 'cancelled' && !vinClubSubscription.cancelledAt && {
...
/**
 * Handle VinClub subscription.deleted event
 * Marks subscription as cancelled
 * @returns true if handled as VinClub subscription, false otherwise
 */
async function handleVinClubSubscriptionDeleted(
  subscription: SubscriptionDeletedData,
  context: any
): Promise<boolean> {
  const stripeSubscriptionId = subscription.id;
